...
- [**treefmt**](https://github.com/numtide/treefmt)
  ...
  ```bash
  treefmt --fail-on-change --no-cache --walk filesystem *.bazel *.bzl *.cjs *.clj *.cljc *.cljs *.cljx *.css *.html *.js *.json *.json5 *.jsonnet *.jsx *.libsonnet *.md *.mdx *.mjs *.nix *.py *.pyi *.rb *.scss *.sh *.tf *.tftest.hcl *.tfvars *.toml *.ts *.tsx *.vue *.yaml *.yml *Vagrantfile
  ```
